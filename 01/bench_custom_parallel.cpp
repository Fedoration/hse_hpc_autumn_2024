...
int main() {
    ...
    double *C_custom_parallel = new double[N * N];
    ...
    init_matrix_zero(N, C_custom_parallel);
    ...
    // Запуск кастомной параллельной версии
    times = omp_get_wtime();

    custom_dgemm(N, A, B, C_blas);

    times = omp_get_wtime() - times;
    std::cout << "Время выполенения кастомной параллельной версии: " << times << std::endl;
    ...
    if (test(N, C_sequential, C_blas)) {
        std::cout << "Результаты совпадают! C_custom_parallel" << std::endl;
    } else {
        std::cout << "Результаты отличаются! Неправильный ответ C_custom_parallel" << std::endl;
    }
